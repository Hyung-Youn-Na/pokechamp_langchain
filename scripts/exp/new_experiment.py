...
def main() -> int:
    ...
    print()
    print("▶ 실행 명령 (본 측정 — 코드 변경 후 사용자 직접 실행, §0-8):")
    print()
    print(build_command(exp_dir_rel, info, args.team_mode, args.team_manifest))
    print()
    print("▶ smoke test 권장 (본 측정 전 — §0-9): --N 1 단일 배틀로")
    print("    회귀·충돌·brief 출력을 먼저 확인. 정상 확인 후 위 본 측정(--N 30) 실행:")
    print()
    print(build_command(exp_dir_rel, info, args.team_mode, args.team_manifest).replace("--N 30", "--N 1"))
    print()
    print("▶ 배틀 후:")
    print(
