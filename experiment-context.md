...
| EXP-055 | react-synthesis-correction-frame (strategy_synthesis 교정 프레임) | 2026-07-01 | ❌ 기각(N=30 역행) | **56.7% (17/30)** | 결정 품질 레버. `react_agent.py` STRATEGY_SYSTEM_PROMPT `## Tool credibility`(oracle=정확/heuristic=추정/충돌 시 oracle 우선) + 헤더 ESTIMATES. screen N=10 **7/10**(vs 050e +1, "가장 유망")에서 N=30 확증 → **17/30 (56.7%)**, vs 050a paired net **-4**(승→패 8), vs 050e **-2**(z<1) 역행. **결정적 교훈: N=10 7/10은 노이즈** — §10.2 gate(8/10) 미달 후보를 N=30로 올린 실수; 30매치업에서 수렴 역행. N=10 gate는 N=30 역행 방지 안전장치 → **미달 후보 N=30 승격 금지**. 결정 품질 단독도 050a 천장 미돌파. **3후보(054/055/056) 전부 기각**, 코드 회수. `docs/analysis/exp-055-react-glm51-analysis.md` |
