...
class LocalSimPool:
    ...
    def initialize_pool(self, template_battle: Battle, **localsim_kwargs):
        ...
        print(
            f"[INIT] Initializing LocalSim pool with {self._initial_size} instance (memory optimized)..."
        )
        for i in range(self._initial_size):
            sim = LocalSim(battle=deepcopy(template_battle), **localsim_kwargs)
            self._available_sims.append(sim)
        print(
            f"[OK] LocalSim pool initialized with {len(self._available_sims)} instance"
        )
    ...
    def acquire_sim(self, battle: Battle) -> LocalSim:
        ...
        if self._available_sims:
            ...
            if self._creation_template is None:
                raise RuntimeError(
                    "Pool not initialized - call initialize_pool() first"
                )

            sim = LocalSim(battle=deepcopy(battle), **self._creation_template)
            self._in_use_sims.append(sim)
            return sim
    ...
    def get_stats(self) -> Tuple[int, int, int]:
        ...
        return (
            len(self._available_sims),
            len(self._in_use_sims),
            len(self._available_sims) + len(self._in_use_sims),
        )
...
class MinimaxCache:
    ...
    def get_evaluation(
        self, battle_state: BattleStateHash, player_action: str, opp_action: str
    ) -> Optional[float]:
...
class OptimizedSimNode:
    ...
    def create_child_node(
        self, player_action: BattleOrder, opp_action: BattleOrder
    ) -> "OptimizedSimNode":
...
class MinimaxOptimizer:
    ...
    def __init__(self):
        self.sim_pool = LocalSimPool(
            initial_size=1
        )  # Single instance to reduce memory usage
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pool_stats": {
                ...
                "in_use": pool_in_use,
                "total": pool_total,
                "reuse_rate": self.stats["pool_reuses"]
                / max(1, self.stats["nodes_created"]),
            },
            "cache_stats": {
                "hits": cache_hits,
                "misses": cache_misses,
