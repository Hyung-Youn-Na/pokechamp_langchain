...
class LLMVGCPlayer(Player):
    def __init__(
        ...
        self.use_damage_calc_early_exit = (
            True  # Use damage calculator to exit early when advantageous
        )
        self.use_llm_value_function = (
            True  # Use LLM for leaf node evaluation (vs fast heuristic)
        )
        self.max_depth_for_llm_eval = (
            2  # Only use LLM evaluation for shallow depths to save time
        )
    ...
    def _send_thinking_message(self, battle: AbstractBattle, message: str):
        ...
        try:
            ...
            async def send_message_async():
                try:
                    print(
                        f"   Sending thinking to battle chat ({len(chunks)} parts)..."
                    )
                    ...
                    for part_num, chunk in enumerate(chunks, 1):
                        ...
                        await self.ps_client.send_message(
                            chat_message, room=battle.battle_tag
                        )
    ...
    def get_LLM_action(
        ...
        if llm is None:
            output, _, raw_message = self.llm.get_LLM_action(
                system_prompt,
                user_prompt,
                model,
                temperature,
            ...
            output, _, raw_message = llm.get_LLM_action(
                system_prompt,
                user_prompt,
                model,
                temperature,
    ...
    def parse_request(self, request: Dict[str, Any]) -> None:
        ...
        if request.get("teamPreview", False) and "side" in request:
            ...
            print(
                f"Stored teampreview team data: {len(self._teampreview_team_data)} Pokemon"
            )
    ...
    def choose_move(self, battle: AbstractBattle):
        ...
        for i, mon in enumerate(battle.active_pokemon):
            if (mon is None or mon.fainted or battle.force_switch[i]) and len(
                battle.available_switches[i]
            ) == 1:
                next_action[i] = BattleOrder(battle.available_switches[i][0])
            elif (
                not (mon is None or mon.fainted)
                and len(battle.available_moves[i]) == 1
                and len(battle.available_switches[i]) == 0
            ):
        ...
        if all(battle.force_switch):
            ...
            if len(total_available_switches) < sum(battle.force_switch):
                print(
                    f"WARNING: Both slots forced to switch but only {len(total_available_switches)} unique switches available"
                )
        ...
        for idx, mon in enumerate(battle.active_pokemon):
            ...
            if battle.force_switch[idx]:
                ...
                if len(battle.available_switches[idx]) == 0:
                    ...
                    print(
                        f"WARNING: Forced to switch but no switches available for slot {idx}"
                    )
                    next_action[idx] = None
                    continue
                ...
                for i, action in enumerate(next_action):
                    if (
                        ...
                        and not isinstance(action, DefaultBattleOrder)
                    ):
                        if hasattr(action, "order") and isinstance(
                            action.order, Pokemon
                        ):
                            already_chosen.append(action.order.species)
                        elif hasattr(action, "order") and hasattr(
                ...
                print(
                    f"DEBUG: Slot {idx} - Already chosen: {already_chosen}, Available: {[p.species for p in battle.available_switches[idx]]}, Filtered: {switches}"
                )

                # If no valid switches left, use first available
                if not switches:
                    switches = [
                        pokemon.species for pokemon in battle.available_switches[idx]
                    ]
                ...
                system_prompt, state_prompt, state_action_prompt = sim.prompt_translate(
                    sim, battle, next_action=next_action, idx=idx
                )
                state_prompt_io = (
                    state_prompt + state_action_prompt + constraint_prompt_io
                )

                retries = 10
                if self.prompt_algo == "io":
                    next_action[idx] = self.io(
                        retries,
                        system_prompt,
                        state_prompt,
                        "",
                ...
                if next_action[idx] is not None and not isinstance(
                    next_action[idx], DefaultBattleOrder
                ):
                    if hasattr(next_action[idx], "order") and isinstance(
                        next_action[idx].order, Pokemon
                    ):
                        ...
                        if is_duplicate:
                            print(
                                f"WARNING: LLM chose duplicate switch {chosen_species}, using fallback"
                            )
                            ...
                            for pokemon in battle.available_switches[idx]:
                                ...
                                next_action[idx] = self.create_order(
                                    battle.available_switches[idx][0]
                                )
            ...
            switches = [
                ...
                not in [
                    ...
                    and not isinstance(action, DefaultBattleOrder)
                    and isinstance(action.order, Pokemon)
                ]
            ]
...
Target numbers: 1=left opponent, 2=right opponent, 0=field effect, 0=self\n"""
        ...
        next_action = DoubleBattleOrder(
            first_order=next_action[0], second_order=next_action[1]
        )
        print(next_action)
        return next_action
    ...
    def io(
        ...
        for i in range(retries):
            try:
                ...
                print(
                    f"LLM [{self.ps_client.account_configuration.username}] Slot {idx+1}: {llm_output}"
                )
                ...
                if "move" in llm_action_json.keys() or is_a_move:
                    ...
                    if actions is not None and len(actions) > 0:
                        ...
                        available_moves_normalized = [
                            move.lower().replace(" ", "") for move in available_moves
                        ]
                    ...
                    for i, move in enumerate(move_list):
                        if move.id.lower().replace(
                            ...
                            next_action = self.create_order(
                                ...
                                dynamax=dynamax,
                                terastallize=tera,
                                move_target=llm_target,
                            )
                            if DEBUG:
                                print(
                                    f"Move match found: {move.id} with target: {llm_target}"
                                )
                            break
                    ...
                    if actions is not None and len(actions) > 1:
                        available_switches = actions[
                            1
                        ]  # Second element is switches list
                        # Convert to lowercase and remove spaces for case-insensitive comparison
                        llm_switch_normalized = llm_switch_species.lower().replace(
                            " ", ""
                        )
                        available_switches_normalized = [
                    ...
                    if DEBUG:
                        ...
                        print(
                            f"Available switches: {[pokemon.species for pokemon in switch_list]}"
                        )
    ...
    def _get_fast_heuristic_evaluation(self, battle_state):
        ...
        try:
            player_hp = (
                int(battle_state.active_pokemon.current_hp_fraction * 100)
                if battle_state.active_pokemon
                else 0
            )
            opp_hp = (
                int(battle_state.opponent_active_pokemon.current_hp_fraction * 100)
                if battle_state.opponent_active_pokemon
                else 0
            )
            player_remaining = len(
                [p for p in battle_state.team.values() if not p.fainted]
            )
            opp_remaining = len(
                [p for p in battle_state.opponent_team.values() if not p.fainted]
            )

            # Use cached fast evaluation
            return fast_battle_evaluation(
                player_hp, opp_hp, player_remaining, opp_remaining, battle_state.turn
            )
        except:
    ...
    def tree_search_optimized(
        ...
        try:
            ...
            system_prompt, state_prompt, _, _, _, _, _ = (
                root.simulation.get_player_prompt(return_actions=True)
            )
                    ...
                    try:
                        ...
                        if "choice" in llm_action_json.keys():
                            if llm_action_json["choice"] != "minimax":
                                ...
                                if return_opp:
                                    try:
                                        action_opp, _ = self.estimate_matchup(
                                            root.simulation,
                                            battle,
            ...
            while len(q) != 0:
                ...
                if node.simulation.is_terminal() or node.depth == self.K:
                    try:
                        ...
                        try:
                            damage_calc_move, damage_calc_turns = self.dmg_calc_move(
                                node.simulation.battle
                            )
                            if damage_calc_turns < float("inf"):
                                ...
                                try:
                                    ...
                                    node.hp_diff = (
                                        50  # Neutral if opponent estimation fails
                                    )
                            else:
                            ...
                            node.hp_diff = node.simulation.get_hp_diff()
                        print(
                            f"LLM value function failed, using damage calculator fallback: {e}"
                        )
                ...
                if node.depth < self.K and player_actions and opponent_actions:
                    for action_p in player_actions[
                        :2
                    ]:  # Limit to 2 player actions for performance
                        for action_o in opponent_actions[
                            :2
                        ]:  # Limit to 2 opponent actions for performance
                            try:
            ...
            print(
                f"[PERF] Optimized minimax: {end_time - start_time:.2f}s, "
                    ...
                    if return_opp:
                        try:
                            ...
                            return dmg_calc_move, (
                                self.create_order(action_opp) if action_opp else None
                            )
                        except:
    ...
    def choose_max_damage_move(self, battle: DoubleBattle, idx: int):
        ...
        if battle.available_moves[idx]:
            best_move = max(
                battle.available_moves[idx], key=lambda move: move.base_power
            )
            return self.create_order(
                best_move, move_target=DoubleBattle.OPPONENT_1_POSITION
            )
    ...
    def teampreview(self, battle: AbstractBattle) -> str:
        ...
        try:
            ...
            available_pokemon = [
                pokemon
                for pokemon in raw_available
                if pokemon and not isinstance(pokemon, list)
            ]
        ...
        Do not repeat the same index more than once."""
            ...
            team_order = self._parse_teampreview_response(
                llm_response, available_pokemon
            )
    ...
    def _format_team_data_for_llm(
        self, available_pokemon: List[Pokemon], opponent_team: List[Pokemon]
    ) -> Dict[str, Any]:
        """Format Pokemon data for LLM analysis."""
        team_data = {"available_pokemon": [], "opponent_pokemon": []}
        ...
        for i, pokemon in enumerate(available_pokemon, 1):
            try:
                pokemon_info = {
                    ...
                    "moves": (
                        [move.name for move in pokemon.moves.values()]
    ...
    def _parse_teampreview_response(
        self, response: str, available_pokemon: List[Pokemon]
    ) -> Optional[str]:
