...
# Mapping from flag key to concise display text (max 30 chars each)
_FLAG_DISPLAY_TEXT: Dict[str, str] = {
...
def get_flag_annotations(move: Move) -> List[str]:
    """Return a list of concise flag annotation strings for a move.

    Only returns annotations for the 7 dynamic flags. Other flags in
    Move.flags are ignored.
    """
    annotations = []
    flags = move.flags
    for flag_key, text in _FLAG_DISPLAY_TEXT.items():
        if flag_key in flags:
            annotations.append(text)
    return annotations


def format_flag_text(move: Move) -> str:
    """Return a single concatenated string of flag annotations for a move.

    Returns empty string if the move has no dynamic flags.
    """
    annotations = get_flag_annotations(move)
    if annotations:
...
def _apply_dynamic_calcs_to_move(
    ...
    # NOTE: oracle dynamic-type/power augmentation now lives in the react
    # battle tools (``battle_tools._resolve_move_outcome_via_oracle``), not in
    # the prompt. Prompt-side oracle injection was an EXP-005/035 noise path
    # (it never reached the sim core / tool layer); ``enable_showdown_oracle``
    # now gates only that single, accurate tool-layer path.
...
def state_translate(
    ...
    for i, move in enumerate(battle.available_moves):
        ...
        # dynamic flag annotations (only when enable_dynamic_flags is True)
        if getattr(sim, "enable_dynamic_flags", False):
...
def state_translate3(
    ...
    if opp_mon is not None:
        ...
        opponent_speed = round(
            opp_stats_slot["spe"] * sim.boost_multiplier("spe", opp_boosts_slot["spe"])
        )
    else:
        opponent_speed = 0
    ...
    for pokemon in battle.opponent_active_pokemon:
        ...
        opponent_move_type_damage_prompt = move_type_damage_wrapper(
            pokemon, sim.gen.type_chart, team_move_type
        )
    ...
    for mon_opp in battle.opponent_team.values():
        if (
            ...
            or mon_opp is None
            or any(
                (pokemon is not None) and (mon_opp.species == pokemon.species)
                for pokemon in battle.opponent_active_pokemon
            )
        ):
            continue
    ...
    if not battle.force_switch[idx] and battle.active_pokemon[idx] is not None:
        ...
        active_hp_fraction = round(
            battle.active_pokemon[idx].current_hp
            / battle.active_pokemon[idx].max_hp
            * 100
        )
        ...
        if battle.active_pokemon[idx].type_1:
            ...
            if battle.active_pokemon[idx].type_2:
                active_type = (
                    active_type
                    + " and "
                    + battle.active_pokemon[idx].type_2.name.capitalize()
                )

        active_move_type_damage_prompt = move_type_damage_wrapper(
            battle.active_pokemon[idx], sim.gen.type_chart, opponent_type_list
        )
        speed_active_stats = active_stats["spe"]
        if speed_active_stats == None:
            speed_active_stats = 0
        ...
        try:
            active_ability = sim.ability_effect[battle.active_pokemon[idx].ability][
                "name"
            ]
            ability_effect = sim.ability_effect[battle.active_pokemon[idx].ability][
                "effect"
            ]
        except:
        ...
        for mon in battle.opponent_active_pokemon:
            if mon is None:
                continue
            speed_prompt += (
                f"(slower than {mon.species})."
                if active_speed < opponent_speed
                else f"(faster than {mon.species})."
            )
    ...
    if not battle.force_switch[idx] and battle.active_pokemon[idx] is not None:
        ...
        for i, move in enumerate(battle.available_moves[idx]):
            ...
            if move.category.name == "SPECIAL":
                ...
                opponent_def = [
                    opp_stats["def"] * sim.boost_multiplier("def", active_boosts["def"])
                    for opp_stats in opponent_stats
                ]
                power = [
        ...
        for _, move in pokemon.moves.items():
            ...
            #     continue # only output attack move
                ...
                for mon in battle.opponent_active_pokemon:
                    ...
                    move_type_damage_prompt += (
                        move_type_damage_wrapper(
                            mon, sim.gen.type_chart, [move.type.name]
                        )
                        + "\n"
                    )
                if "2x" in move_type_damage_prompt:
    ...
    if (
        not (battle.active_pokemon[idx] is None)
        and battle.active_pokemon[idx].species in switch_choices
    ):
    ...
    if (
        ...
        or battle.active_pokemon[idx] is None
        or battle.active_pokemon[idx].fainted
    ):  # passive switching
