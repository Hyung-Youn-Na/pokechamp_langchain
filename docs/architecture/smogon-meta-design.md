...
| **★ overview 96종 빈 (EXP-049c 발견, 2026-06-25)** | 108종 중 96종의 `overview`가 빈 문자열. 추적 결과 Smogon dex `_rpc` `dump-pokemon` API가 OU format의 overview를 빈으로 반환 (crawler `normalize()` / 정제 / 도구 로직은 버그 아님). overview 있는 12종만(gliscor·iron-valiant·zamazenta·dragonite·kingambit·gholdengo·landorus-therian·ting-lu·kyurem·deoxys-speed·samurott-hisui·ogerpon-wellspring) 풀 C&C 분석. | `overview` 빈 → `get_strategy_insight` 도구가 88.9% 빈 overview 반환 → **EXP-049c "방식 1 기각" 오평가의 진짜 원인** (glm-5.1 탓 아님). **복구**: crawler `normalize()`에 Draft format "Overview:" 섹션 fallback 추가 — toxapex/rillaboom 검증 672/610자 채워짐. 빈 overview 종의 Draft overview에서 포켓몬 일반 역할 서술 추출 (BSS는 format 소개라 제외). |
| **comments(Other Options) 96종 빈** | overview와 **항상 세트** — overview 있는 동일 12종만 comments 有. (이전 기록 "comments 96종 빈"은 사실이나, **overview 96종 빈을 동반**함을 명시.) | `other_options` 빈 문자열. |
