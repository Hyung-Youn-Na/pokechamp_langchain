...
> ⚠️ **EXP-049b 업데이트 (2026-06-29)**: 그래프가 5노드(`tool_agent` / `strategy_synthesis` 분리)로
> 변경되어 라우팅이 `"tools"` / `"strategy_synthesis"`입니다. 본문의 `"parse"` 라우팅과
> `sum(ToolMessage)` 카운터는 구버전 — 현재는 state의 `tool_call_count`(Annotated) 카운터 기반이며,
> 강제종료는 `agent_loop`가 아닌 **`strategy_synthesis` 노드의 clean rebuild**로 처리됩니다.
> 정확한 구현은 `react_agent.py`(`_make_should_continue` L539-565, `_make_strategy_synthesis` L256-335).
...
```
build_context → tool_agent ⇄ tool_execution → strategy_synthesis → parse_action → END
                    ↓
              [should_continue]   (EXP-049b)
              ├─ "tools" → tool_execution (루프 계속)
              └─ "strategy_synthesis" → clean rebuild (루프 종료)
```
...
| # | 규칙 | 조건 | 결과 |
|---|------|------|------|
| 1 | Tool call count 한도 | state `tool_call_count` >= `max_tool_calls` | `"strategy_synthesis"` (clean rebuild) |
| 2 | 남은 예산 없음 | `max_tool_calls - count` <= 0 | `"strategy_synthesis"` (clean rebuild) |
| 3 | AI 메시지에 tool_calls 존재 | `last_message.tool_calls`가 비어있지 않음 | `"tools"` (루프 계속) |
| 4 | tool_calls 없음 | `last_message.tool_calls`가 없거나 비어있음 | `"strategy_synthesis"` (clean rebuild) |
...
**EXP-049b 이관:** 과거 `agent_loop`(react_agent.py 구버전)에서 작동하던 강제종료(clean rebuild +
JSON mode) 로직은 **`strategy_synthesis` 노드**(`_make_strategy_synthesis`, react_agent.py:256-335)로
이관되었습니다. `should_continue`가 예산 도달 시 곧장 `"strategy_synthesis"`로 라우팅하면, 이 노드가
툴 결과 + 마지막 추론만 추려 clean rebuild로 `STRATEGY_SYSTEM_PROMPT` + `response_format=json_object`로
최종 결정을 내립니다. 본문의 `agent_loop` 내 강제종료 묘사는 구버전입니다.
