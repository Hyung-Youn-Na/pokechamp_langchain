...
### 1.2 실배틀 compare (사용자 실행 후 보강 예정)

```sh
.venv/bin/python scripts/battles/local_1v1_langchain.py ... --team_mode fixed \
  --team_manifest .temp/experiments/fixed-baselines/manifests/dynamic-v2.json \
  --enable_showdown_oracle --enable_llm_lead_selection --oracle_backend compare --N 30 \
  --log_dir .temp/experiments/active/EXP-057-damage-backend-compare/battle_log
.venv/bin/python scripts/exp/aggregate_oracle_compare.py .temp/experiments/active/EXP-057-damage-backend-compare
```
_(본 보고서는 제어 스위트로 1차 결론; 실배틀 집계 결과는 1.2에 추가 예정.)_
...
**데미지 정확도**: type 100% 일치. 데미지는 (A) KO clamp(측정방식차, 자연스러움) + (B) 동적 무브(영역별 우위 상이). **동적 무브/tera 영역에서는 calc가 더 견고**(Tera Blast 정상·동적 위력 일관) — 단 "calc가 전반적으로 더 정홠"은 아니고, oracle은 KO clamp(실제 전투 동작) 우위. **둘 다 특정 무브에서 각각 실패 모드**(oracle: Tera Blast=0; calc: worker 변환 누락 3종 — 본 EXP에서 수정).

**편리성**: **calc 압도적 우위** — 런타임 산물 42배 작고, 빌드 1단계/2.6s, 의존성 경량, 결정론적, 재현성 양호.

**권고**: 
- 데미지 정확도(동적 무브 견고성) + 편리성 양축에서 calc 우위 → **calc 백엔드로의 전환 근거 충분**.
- 단, 승률 영향은 본 실험 범위 밖(사용자 결정). 전환은 **별도 ablation**(`--oracle_backend damagecalc` 단독, 승률 게이트)에서 측정. KO clamp 차이가 battle_tools의 median 사용에 미치는 영향(050a의 "거짓 생존 유리" 패턴, EXP-054 교훈)이 승률에 어떻게 작용할지가 열쇠.
- worker 변환 버그 3종(toMoveName/toSpeciesName)은 calc 전환 여부와 무관하게 **즉시 수정 가치**(현재 compare/showdown 모두에 영향).
