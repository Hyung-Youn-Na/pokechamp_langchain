            ...
            try:
                ...
                if bayesian_result and len(bayesian_result) >= len(opponent_moves):
                    if return_separate:
                        return (
                            opponent_moves,
                            bayesian_result[len(opponent_moves) :],
                        )  # Separate confirmed vs predicted
                    return bayesian_result[
                        :4
                    ]  # Return top 4 Bayesian moves (includes 100% confirmed moves)
            except:
    ...
    def _get_bayesian_move_predictions(self, mon):
        ...
        try:
            ...
            probabilities = predictor.predict_component_probabilities(
                species_norm, teammates=opponent_pokemon, observed_moves=observed_moves
            )
        ...
        try:
            if return_actions:
                return self.prompt_translate(
                    self, battle, return_actions=return_actions, idx=idx
                )
            if return_choices:
                return self.prompt_translate(
                    self, battle, return_choices=return_choices, idx=idx
                )
            ...
            if return_actions:
                return self.prompt_translate(
                    self, battle, return_actions=return_actions
                )
            if return_choices:
                return self.prompt_translate(
                    self, battle, return_choices=return_choices
                )
